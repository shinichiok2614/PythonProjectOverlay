...
COLOR_CPU = "#00BFFF"
COLOR_NET = "lime"
COLOR_GRAPH_CPU = "#00BFFF"
COLOR_GRAPH_NET = "lime"
COLOR_SHADOW = "#111"

GRAPH_WIDTH = 120
GRAPH_HEIGHT = 30
GRAPH_POINTS = 60  # số điểm lưu
...
def update():
    ...
    # ===== TIME =====
    draw_text(0, y, now.strftime("%H:%M:%S"), COLOR_TIME, FONT_TIME)
    y += FONT_TIME

    draw_text(0, y, f"{weekday[now.weekday()]} {now.day:02d}/{now.month:02d}", COLOR_TIME, FONT_DATE)
    y += FONT_DATE + 6

    # ===== CPU =====
    draw_text(0, y, f"CPU {cpu:>3}%", COLOR_CPU, FONT_INFO)
    y += FONT_INFO + 2

    draw_graph(0, y, cpu_history, COLOR_GRAPH_CPU)
    y += GRAPH_HEIGHT + 4

    # ===== RAM =====
    draw_text(0, y, f"RAM {ram:>3}%", COLOR_CPU, FONT_INFO)
    y += FONT_INFO + 6

    # ===== NETWORK =====
    if net_name:
        draw_text(0, y, net_name, COLOR_NET, FONT_INFO)
        y += FONT_INFO + 2

        draw_text(0, y, f"↓ {format_speed(down)}", COLOR_NET, FONT_INFO)
        y += FONT_INFO + 2

        draw_graph(0, y, net_history, COLOR_GRAPH_NET)
        y += GRAPH_HEIGHT + 2
